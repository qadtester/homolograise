        ...
        if not risks:
            ...
            if can_edit(user_info):
                ...
                for r in risks:
                    ...
                    with st.expander(f"{score_color} [{r.get('risk_type', 'Produto')}] Risco (Score: {score}) - {str(r.get('risk_description', ''))[:50]}... [{ai_badge}]"):
                        ...
                        with st.form(key=f"form_edit_risk_{r['id']}"):
                            ...
                            with ec1:
                                e_type = st.selectbox("Tipo", ["Produto", "Projeto"], index=0 if r.get('risk_type') == "Produto" else 1, key=f"e_type_{r['id']}")
                            ...
                            with ec3:
                                e_owner = st.text_input("Dono do Risco", value=r.get('risk_owner', ''), key=f"e_owner_{r['id']}")
                                e_period = st.text_input("Periodicidade", value=r.get('periodicity', ''), key=f"e_period_{r['id']}")

                            e_plan = st.text_area("Como Corrigir", value=r.get('correction_plan', ''), key=f"e_plan_{r['id']}")
