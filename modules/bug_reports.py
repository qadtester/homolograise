...
def render_bug_reports_tab(project_id: str):
                        ...
                        if raw_bug.strip():
                            with st.spinner("Gerando Bug Report..."):
                                ...
                                if data and isinstance(data, dict):
                                    ...
                                    if res.data:
                                        create_kanban_card_from_bug(project_id, res.data[0]["id"], payload["title"], payload["description"], payload["severity"])
                ...
                with st.form("bug_report_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("🚨 Registrar Bug"):
                        if not active_bug_cycle:
                            ...
                            res = supabase.table("bug_reports").insert(payload).execute()
                            if res.data:
                                create_kanban_card_from_bug(project_id, res.data[0]["id"], payload["title"], payload["description"], severity)
    ...
    if bug_cycle_filter != "Todos": b_query = b_query.eq("test_cycle", bug_cycle_filter)
    if bug_status_filter != "Todos": b_query = b_query.eq("status", bug_status_filter)
    if bug_severity_filter != "Todos": b_query = b_query.eq("severity", bug_severity_filter)
                        ...
                        with c_edit:
                            if can_edit(user_info):
                                with st.popover("✏️ Editar", key=f"pop_edit_bug_{bug['id']}"):
                                    ...
                                    if st.button("Salvar Edições", key=f"btn_bug_edit_{bug['id']}"):
                                        supabase.table("bug_reports").update({"title": e_title, "description": e_desc}).eq('id', bug['id']).execute()
                                        st.rerun()

                        with c_clone:
                            if can_edit(user_info) and st.button("📋 Duplicar", key=f"btn_bug_clone_{bug['id']}"):
                                bug_clone_payload = {**bug, "title": f"{bug['title']} (Cópia)", "status": "Aberto"}
                                bug_clone_payload.pop("id", None)
                                res = supabase.table("bug_reports").insert(bug_clone_payload).execute()
                                if res.data:
                                    create_kanban_card_from_bug(project_id, res.data[0]["id"], bug_clone_payload["title"], bug_clone_payload.get("description", ""), sev)
                                st.rerun()

                        with c_del:
                            if can_delete_items(user_info) and st.button("🗑️ Excluir", key=f"btn_bug_del_{bug['id']}", type="primary"):
