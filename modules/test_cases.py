...
def render_test_cases_tab(project_id: str):
    ...
    try:
        ...
        if filter_cycle != "Todos": query = query.eq("test_cycle", filter_cycle)
        if filter_type != "Todos": query = query.eq("test_type", filter_type)
        if filter_status != "Todos": query = query.eq("status", filter_status)
    ...
    if not test_cases:
        ...
        with col_dl1:
            st.download_button("📥 CSV", export_to_csv(test_cases).encode("utf-8-sig"), f"tc_{project_id[:8]}.csv", "text/csv")
        with col_dl2:
            st.download_button("📥 Markdown", export_to_markdown(test_cases, title=f"TCs - {filter_cycle}"), f"tc_{project_id[:8]}.md", "text/markdown")
        with col_dl3:
            st.download_button("🌐 HTML", export_to_html(test_cases, title=f"TCs - {filter_cycle}").encode("utf-8"), f"tc_{project_id[:8]}.html", "text/html")
                ...
                for tc in tc_list:
                    ...
                    with st.expander(f"{status_icon} [{tc.get('test_type', 'Teste')}] - {tc.get('title')}"):
                                ...
                                if can_edit(user_info):
                                    with st.popover("✏️ Editar", key=f"pop_edit_tc_{tc['id']}"):
                                        ...
                                        if st.button("Salvar Alterações", key=f"btn_tc_edit_{tc['id']}"):
                                            supabase.table("test_cases").update({"title": e_title, "steps": e_steps, "expected_result": e_exp}).eq('id', tc['id']).execute()
                                            st.rerun()
                            with c_clone:
                                if can_edit(user_info) and st.button("📋 Duplicar", key=f"btn_tc_clone_{tc['id']}"):
                                    clone_payload = {**tc, "id": None, "title": f"{tc['title']} (Cópia)", "status": "Não Executado"}
                                    clone_payload.pop("id", None)
                                    supabase.table("test_cases").insert(clone_payload).execute()
                                    st.rerun()
                            with c_del:
                                if can_delete_items(user_info) and st.button("🗑️ Excluir", key=f"btn_tc_del_{tc['id']}", type="primary"):
                        ...
                        with col_act:
                            ...
                            if can_edit(user_info):
                                for st_name in ["🟢 Passou", "🔴 Falhou", "🟡 Bloqueado"]:
                                    val = st_name.split()[1]
                                    if st.button(st_name, key=f"btn_st_{val}_{tc['id']}", use_container_width=True):
                                        supabase.table("test_cases").update({"status": val}).eq("id", tc['id']).execute()
                                        st.rerun()
